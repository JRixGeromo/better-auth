...
async function setupDatabase() {
  ...
  try {
    ...
    // Create schema using Supabase stored procedure
    console.log('\n2️⃣ Creating schema...');
    ...
    if (procError) {
      console.log('⚠️ Could not create stored procedure');
    ...
    if (execError) {
      console.log('⚠️ Could not execute stored procedure');
      console.log('Error:', execError.message);
      return;
    }

    // Verify schema exists by querying information_schema
    ...
    if (!verifyError && schemaData) {
      console.log('✅ Verified better_auth schema exists');
      console.log('\nNext steps:');
      console.log('1. Run BetterAuth migrations');
      console.log('2. Start your application');
    } else {
      console.log('⚠️ Could not verify schema directly');
    ...
    console.log('   - Create schema manually in SQL editor:');
    console.log('     CREATE SCHEMA IF NOT EXISTS better_auth;');
    console.log('     GRANT USAGE ON SCHEMA better_auth TO postgres;');
    console.log('\n2. If problems persist:');
    console.log('   - Check Supabase logs for database errors');
    console.log('   - Try restarting the database');
    console.log('   - Verify your IP is not being blocked');
